...
static std::string start_py_template() {
    return
        ...
        "def get_venv_env():\n"
        "    \\\"\\\"\\\"Create environment dict with venv paths so subprocess uses venv packages.\\\"\\\"\\\"\n"
        "    env = os.environ.copy()\n"
        "    \n"
        "    if os.name == \"nt\":\n"
        "        # Windows: prepend venv/Scripts to PATH\n"
        "        venv_scripts = ROOT / \"oxsium\" / \"Scripts\"\n"
        "        env[\"PATH\"] = str(venv_scripts) + os.pathsep + env.get(\"PATH\", \"\")\n"
        "    else:\n"
        "        # Unix: prepend venv/bin to PATH\n"
        "        venv_bin = ROOT / \"oxsium\" / \"bin\"\n"
        "        env[\"PATH\"] = str(venv_bin) + os.pathsep + env.get(\"PATH\", \"\")\n"
        "    \n"
        "    # Set VIRTUAL_ENV so Python knows it's in a venv\n"
        ...
        "    # Get venv environment for subprocess\n"
