...
static std::string start_py_template() {
    return
        ...
        "\n"
        "\n"
        "# --- Self-relaunch bootstrap -----------------------------------------\n"
        "# This file may be started with ANY Python on PATH (system python,\n"
        "# the py launcher, etc). The function below automatically re-execs\n"
        "# itself using the 'oxsium' venv's python.exe if it isn't already\n"
        "# running under it. This means plain  .\\start.py  (or  python start.py)\n"
        "# always ends up using the correct interpreter and libraries, with no\n"
        "# need to manually activate the venv or remember its path.\n"
        ...
        "        # venv not found yet - fall back to whatever Python launched\n"
        "        # this script. The import below will fail with a clear error\n"
        "        # telling the user to run setup.exe first.\n"
        "        return\n"
        "\n"
        "    try:\n"
        ...
        "        current = None\n"
        "\n"
        "    if current is not None and current == venv_py.resolve():\n"
        "        return  # already running under the correct interpreter\n"
        "\n"
        "    # Re-launch this exact script using the oxsium venv's python.exe.\n"
