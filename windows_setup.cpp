...
static std::string start_py_template() {
    return
        ...
        "\n"
        "# ── Resolve venv Python early (before NiceGUI import) ────────────────────────\n"
        "def _venv_python_path() -> Path:\n"
        "    \"\"\"Return venv python.exe path if it exists, else current interpreter.\"\"\"\n"
        ...
        "\n"
        "\n"
        "def _ensure_nicegui() -> None:\n"
        "    \"\"\"\n"
        "    Make sure nicegui is importable in the CURRENT interpreter.\n"
        "    Priority:\n"
        "      1. Already importable — done.\n"
        "      2. Venv site-packages exist — add them to sys.path so the\n"
        "         venv's nicegui is found without re-installing anything.\n"
        "      3. Last resort — pip-install into the current interpreter.\n"
        "    \"\"\"\n"
        "    if importlib.util.find_spec(\"nicegui\") is not None:\n"
        "        return  # already on path\n"
        "\n"
        "    # Try to locate venv site-packages and inject them\n"
        "    root = Path(__file__).resolve().parent\n"
        "    venv_dir = root / \"oxsium\"\n"
        "    if venv_dir.exists():\n"
        "        import glob as _glob\n"
        "        patterns = [\n"
        "            str(venv_dir / \"Lib\" / \"site-packages\"),                          # Windows\n"
        "            str(venv_dir / \"lib\" / \"python*\" / \"site-packages\"),              # Linux/macOS\n"
        "        ]\n"
        "        for pat in patterns:\n"
        ...
        "        if importlib.util.find_spec(\"nicegui\") is not None:\n"
        "            return  # found via venv site-packages\n"
        "\n"
        "    # Venv missing or nicegui not installed there — install into current Python\n"
        "    print(\"  [SETUP] nicegui not found in venv — installing into current Python...\")\n"
        ...
        "        ui.html('<div class=\"ox-logo-mark\">OX</div>')\n"
        "        ui.html('<div class=\"ox-title\">Oxsium Framework</div>')\n"
